/**
 * avatarkit 0.2.0 — geometric 3D avatars in SVG.
...
const SCHEMA_ID = "avatarkit.scene";
/** What `SCHEMA_ID` was before 0.2.0. Read, never written — see `migrate`. */
const LEGACY_SCHEMA_ID = "character.scene";
const VERSION = 1;
...
function cloneScene(doc) {
  ...
  return { SCHEMA_ID, LEGACY_SCHEMA_ID, VERSION, UNIT, LIMITS, EYE_SHAPES, MOUTH_SHAPES, NOSE_SHAPES, FRAMES, FITS, BACKGROUND_STYLES, PART_ROLES, defaultPart, defaultScene, normalize, migrate, cloneScene };
})();
 ...
 */
function createAvatar(target, document_, options) {
  const opts = options || {};
...
function renderToString(document_, options) {
  ...
  return { createAvatar, renderToString };
})();
...
M["src/index.js"] = (function () {
  const { VERSION, SCHEMA_ID, LEGACY_SCHEMA_ID, UNIT, LIMITS, EYE_SHAPES, MOUTH_SHAPES, NOSE_SHAPES, FRAMES, FITS, BACKGROUND_STYLES, PART_ROLES, defaultScene, defaultPart, normalize, migrate, cloneScene } = M["src/schema.js"];
  ...
  const { createAvatar, renderToString } = M["src/renderer.js"];
...
// to drive the pose itself should use `instance.setPose()` instead.
  ...
  return { VERSION, SCHEMA_ID, LEGACY_SCHEMA_ID, UNIT, LIMITS, EYE_SHAPES, MOUTH_SHAPES, NOSE_SHAPES, FRAMES, FITS, BACKGROUND_STYLES, PART_ROLES, defaultScene, defaultPart, normalize, migrate, cloneScene, PALETTES, PALETTE_IDS, palette, mix, grade, contrast, luminance, readableInk, hexToRgb, rgbToHex, SHAPES, SHAPE_LABELS, PRESETS, PRESET_IDS, preset, applyPreset, generateScene, randomScene, buildRenderModel, viewMatrix, VIEWBOX, toSVG, toSVGBody, toDataURL, createAvatar, renderToString, FollowController, _setPointer, _activeCount, encode, decode, fromURL, toURL, toPNGBlob, svgToPNGBlob, downloadSVG, downloadPNG, downloadBlob };
})();
...
const FACE_PRESETS = [
  ...
  return {
    ...
    LEGACY_SCHEMA_ID: M["src/index.js"].LEGACY_SCHEMA_ID,
    ...
    createAvatar: M["src/index.js"].createAvatar,
